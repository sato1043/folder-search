...
実装に着手する前に、関連するドキュメントを先に更新する。

- 設計文書（`docs/architecture.md`）に新しいモジュールやフローを追記する
- `docs/TODO.md` に設計方針・検討事項を記載する
- 設計意図を明文化してから実装に入ることで、方針のブレを防ぐ

実装完了後に再度ドキュメントを見直し、実装結果と乖離があれば修正する。
...
- [ ] 実装着手前に設計文書（architecture.md等）を更新した
- [ ] 実装完了後にドキュメントと実装の乖離がないか確認した
