...
- Embeddingモデルの起動時自動ダウンロード（未DL時にオーバーレイ付きで自動取得、LLM自動ロードと並列実行）
- 設定ダイアログの一般セクションにベクトル検索モデルの状態表示・ダウンロードボタンを追加
- 起動時インデックス破損検査（バックグラウンドで全キャッシュを低優先度検証、破損時は自動削除）
- フォルダ選択時のインデックス同期検証（バックグラウンド検証との競合制御付き）
...
- ハイブリッド検索をデフォルト化（ベクトルインデックス未構築時は全文検索に自動フォールバック）
- 検索モード切替UI（全文検索/ハイブリッド検索ラジオボタン）を削除
- サイドバーのタイトル・ボタン類を削除し、フォルダ選択・設定ボタンを検索バー右隣のアイコンに移動
...
- `build_index`, `build_vector_index` コマンドを async + spawn_blocking に変更（インデックス構築中のUI固まりを解消）
